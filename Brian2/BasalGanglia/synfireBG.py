...
Cortex=NeuronGroup(n, synfireEqs,threshold='v > 30',
                    reset=synfireReset, 
                    method='euler')
Cortex.v = c5
Cortex.u = b5*c5
...
# MSN plasticity
traceTau = traceTauPost = 60*ms # this will be integration window 
traceConstant = 0.1 # this will be the decay?
...
MSNstdp=  '''
             ...
             dapre/dt = -apre/taupre : 1 (event-driven)
             dapost/dt = -apost/taupost : 1 (event-driven)
             tracepost/dt = -traceCon/traceTau : 1 (event-driven)
             '''    
MSNpre='''
             v_post += w*mV
             apre += Apre
...
Cortex_D1 = Synapses(Cortex,D1,MSN,on_pre=onpre,on_post=onpost)
...
Thal_Cortex = Synapses(Thalamus,Cortex,'w:1',on_pre='v+=1.75')
...
CortexD1Ind = StateMonitor(Cortex_D1,'i',record=Cortex_D1['i!=j'])
#CortexD1spikes = SpikeMonitor(Cortex_D1)


Cortex_D2mon = StateMonitor(Cortex_D2,'w', record=Cortex_D2['i!=j'])
CortexD2Ind = StateMonitor(Cortex_D2,'i',record=Cortex_D2['i!=j'])


#for cellNum in range(0,len(N)):
#    Cortex_D2mon.w[cellNum, activeSynapses]
#%%
#####333
