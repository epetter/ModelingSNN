...
DA_duration = 100*ms # test to see if DA neurons fire
MSN_SNr_duration = 100*ms # test to see how MSNs affect SNr activity
binSize = 100*ms 
...
############ SNr Neurons 
SNrL = NeuronGroup(n+10,SNrEqs,threshold='v>SNr_spike',reset=SNrReset,method='euler') # Humphries, et al., 2006 

SNrL.v = SNr_c
SNrL.u = SNr_b*SNr_c

SNrNL = NeuronGroup(n+10,SNrEqs,threshold='v>SNr_spike',reset=SNrReset,method='euler') # Humphries, et al., 2006 

SNrNL.v = SNr_c
SNrNL.u = SNr_b*SNr_c

SNrWL = NeuronGroup(n+10,SNrEqs,threshold='v>SNr_spike',reset=SNrReset,method='euler') # Humphries, et al., 2006 

SNrWL.v = SNr_c
SNrWL.u = SNr_b*SNr_c

############ STN Neurons 
STN = NeuronGroup(n*numChannels,STNeqs,threshold='v>STN_spike',reset=STNreset,method='euler') # Humphries, et al., 2006 
...
CortexL_D1L.connect(j='k for k in range(i-w2, i+w2) if rand()<Cortex_MSN_prob', skip_if_invalid=True)
...
CortexNL_D1NL.connect(j='k for k in range(i-w2, i+w2) if rand()<Cortex_MSN_prob', skip_if_invalid=True)
...
CortexWL_D1WL.connect(j='k for k in range(i-w2, i+w2) if rand()<Cortex_MSN_prob', skip_if_invalid=True)
...
CortexL_D2.connect(j='k for k in range(i-w2, i+w2) if rand()<Cortex_MSN_prob', skip_if_invalid=True)
CortexL_D2.delay = 10*ms # Humphries, et al., 2006 
CortexL_D2.w = s

CortexNL_D2 = Synapses(CortexNL,D2_NL,weightEqs,on_pre=addW) #on_pre='v=+10')
CortexNL_D2.connect(j='k for k in range(i-w2, i+w2) if rand()<Cortex_MSN_prob', skip_if_invalid=True)
CortexNL_D2.delay = 10*ms # Humphries, et al., 2006 
CortexNL_D2.w = s

CortexWL_D2 = Synapses(CortexWL,D2_WL,weightEqs,on_pre=addW) #on_pre='v=+10')
CortexWL_D2.connect(j='k for k in range(i-w2, i+w2) if rand()<Cortex_MSN_prob', skip_if_invalid=True)
...
CortexL_STN.connect(j='k for k in range(i-w2, i+w2) if rand()<Cortex_STN_prob', skip_if_invalid=True)
...
CortexNL_STN.connect(j='k for k in range(i-w2, i+w2) if rand()<Cortex_STN_prob', skip_if_invalid=True)
CortexNL_STN.delay = 2.5*ms # Humphries, et al., 2006 
CortexNL_STN.w = d

CortexWL_STN = Synapses(CortexWL,STN,weightEqs,on_pre=addW)
CortexWL_STN.connect(j='k for k in range(i-w2, i+w2) if rand()<Cortex_STN_prob', skip_if_invalid=True)
...
DA_D1L.connect(j='k for k in range(i-w2, i+w2) if rand()<DA_MSN_prob', skip_if_invalid=True) 
DA_D1L.delay = 5*ms
DA_D1L.w = s#rand(len(DA_D1L.i))

DA_D1NL = Synapses(DA,D1_NL,DAstdp,on_pre=onpreDA_D1,on_post=onpostDA)
DA_D1NL.connect(j='k for k in range(i-w2, i+w2) if rand()<DA_MSN_prob', skip_if_invalid=True) 
DA_D1NL.delay = 5*ms
DA_D1NL.w = s#rand(len(DA_D1NL.i))

DA_D1WL = Synapses(DA,D1_WL,DAstdp,on_pre=onpreDA_D1,on_post=onpostDA)
DA_D1WL.connect(j='k for k in range(i-w2, i+w2) if rand()<DA_MSN_prob', skip_if_invalid=True) 
DA_D1WL.delay = 5*ms
DA_D1WL.w = s#rand(len(DA_D1WL.i))

# DA projections to D1
DA_D2L = Synapses(DA,D2_L,DAstdp,on_pre=onpreDA_D2,on_post=onpostDA)
DA_D2L.connect(j='k for k in range(i-w2, i+w2) if rand()<DA_MSN_prob', skip_if_invalid=True) 
DA_D2L.delay = 5*ms
DA_D2L.w = s#rand(len(DA_D1L.i))

DA_D2NL = Synapses(DA,D2_NL,DAstdp,on_pre=onpreDA_D2,on_post=onpostDA)
DA_D2NL.connect(j='k for k in range(i-w2, i+w2) if rand()<DA_MSN_prob', skip_if_invalid=True) 
DA_D2NL.delay = 5*ms
DA_D2NL.w = s#rand(len(DA_D1NL.i))

DA_D2WL = Synapses(DA,D2_WL,DAstdp,on_pre=onpreDA_D2,on_post=onpostDA)
DA_D2WL.connect(j='k for k in range(i-w2, i+w2) if rand()<DA_MSN_prob', skip_if_invalid=True) 
DA_D2WL.delay = 5*ms
...
GPeL_SNrL.connect(j='k for k in range(i-w2, i+w2) if rand()<GPe_SNr_prob', skip_if_invalid=True) 
...
GPeNL_SNrNL.connect(j='k for k in range(i-w2, i+w2) if rand()<GPe_SNr_prob', skip_if_invalid=True) 
...
GPeWL_SNrWL.connect(j='k for k in range(i-w2, i+w2) if rand()<GPe_SNr_prob', skip_if_invalid=True) 
...
GPe_L_STN.connect(j='k for k in range(i-w2, i+w2) if rand()<GPe_STN_prob', skip_if_invalid=True) 
...
GPe_NL_STN.connect(j='k for k in range(i-w2, i+w2) if rand()<GPe_STN_prob', skip_if_invalid=True) 
...
GPe_WL_STN.connect(j='k for k in range(i-w2, i+w2) if rand()<GPe_STN_prob', skip_if_invalid=True) 
...
# Thalamus
SNr_Thalamus_prob = 0.33 
SNrL_ThalamusL = Synapses(SNrL,ThalamusL,weightEqs,on_pre=subW)
SNrL_ThalamusL.connect(j='k for k in range(i-w2, i+w2) if rand()<SNr_Thalamus_prob', skip_if_invalid=True)
...
SNrNL_ThalamusNL.connect(j='k for k in range(i-w2, i+w2) if rand()<SNr_Thalamus_prob', skip_if_invalid=True)
SNrNL_ThalamusNL.delay = 5*ms
SNrNL_ThalamusNL.w = d

SNrWL_ThalamusWL = Synapses(SNrWL,ThalamusWL,weightEqs,on_pre=subW)
SNrWL_ThalamusWL.connect(j='k for k in range(i-w2, i+w2) if rand()<SNr_Thalamus_prob', skip_if_invalid=True)
...
STN_SNrL.connect(j='k for k in range(i-w2, i+w2) if rand()<STN_SNr_prob', skip_if_invalid=True)
STN_SNrL.delay = 1.5*ms # Humphries, et al., 2006 
STN_SNrL.w = p

STN_SNrNL = Synapses(STN,SNrNL,weightEqs,on_pre=addW)
STN_SNrNL.connect(j='k for k in range(i-w2, i+w2) if rand()<STN_SNr_prob', skip_if_invalid=True)
STN_SNrNL.delay = 1.5*ms # Humphries, et al., 2006 
STN_SNrNL.w = p

STN_SNrWL = Synapses(STN,SNrWL,weightEqs,on_pre=addW)
STN_SNrWL.connect(j='k for k in range(i-w2, i+w2) if rand()<STN_SNr_prob', skip_if_invalid=True)
...
STN_GPe_L.connect(j='k for k in range(i-w2, i+w2) if rand()<STN_GPe_prob', skip_if_invalid=True)  
...
STN_GPe_NL.connect(j='k for k in range(i-w2, i+w2) if rand()<STN_GPe_prob', skip_if_invalid=True)  
...
STN_GPe_WL.connect(j='k for k in range(i-w2, i+w2) if rand()<STN_GPe_prob', skip_if_invalid=True)  
STN_GPe_WL.delay = 2*ms # Humphries, et al., 2006 
STN_GPe_WL.w = p # Humphries, et al., 2006... 

############ Thalamus Projections 
Thal_Cortex_prob = 0.3
ThalCortexL = Synapses(ThalamusL,CortexL,weightEqs,on_pre=addW)
ThalCortexL.connect(j='k for k in range(i-w2, i+w2) if rand()<Thal_Cortex_prob', skip_if_invalid=True) #connect(i=[0,1,2],j=[0,1,2])# j='k for k in range(i-w2, i+w2) if rand()<0.33', skip_if_invalid=True)
ThalCortexL.delay = 5*ms 
ThalCortexL.w = p

ThalCortexNL = Synapses(ThalamusNL,CortexNL,weightEqs,on_pre=addW)
ThalCortexNL.connect(j='k for k in range(i-w2, i+w2) if rand()<Thal_Cortex_prob', skip_if_invalid=True) #connect(i=[0,1,2],j=[0,1,2])# j='k for k in range(i-w2, i+w2) if rand()<0.33', skip_if_invalid=True)
ThalCortexNL.delay = 5*ms 
ThalCortexNL.w = p

ThalCortexWL = Synapses(ThalamusWL,CortexWL,weightEqs,on_pre=addW)
ThalCortexWL.connect(j='k for k in range(i-w2, i+w2) if rand()<Thal_Cortex_prob', skip_if_invalid=True) #connect(i=[0,1,2],j=[0,1,2])# j='k for k in range(i-w2, i+w2) if rand()<0.33', skip_if_invalid=True)
ThalCortexWL.delay = 5*ms 
ThalCortexWL.w = p

Thalamus_D1_prob = 0.3
Thal_D1L = Synapses(ThalamusL,D1_L,weightEqs,on_pre=addW)
Thal_D1L.connect(j='k for k in range(i-w2, i+w2) if rand()<Thalamus_D1_prob', skip_if_invalid=True) #connect(i=[0,1,2],j=[0,1,2])# j='k for k in range(i-w2, i+w2) if rand()<0.33', skip_if_invalid=True)
Thal_D1L.delay = 5*ms 
Thal_D1L.w = p

Thal_D1NL = Synapses(ThalamusL,D1_NL,weightEqs,on_pre=addW)
Thal_D1NL.connect(j='k for k in range(i-w2, i+w2) if rand()<Thalamus_D1_prob', skip_if_invalid=True) #connect(i=[0,1,2],j=[0,1,2])# j='k for k in range(i-w2, i+w2) if rand()<0.33', skip_if_invalid=True)
Thal_D1NL.delay = 5*ms 
Thal_D1NL.w = p

Thal_D1WL = Synapses(ThalamusL,D1_WL,weightEqs,on_pre=addW)
Thal_D1WL.connect(j='k for k in range(i-w2, i+w2) if rand()<Thalamus_D1_prob', skip_if_invalid=True) #connect(i=[0,1,2],j=[0,1,2])# j='k for k in range(i-w2, i+w2) if rand()<0.33', skip_if_invalid=True)
